/**
 * #4145 — migration v132 (minion_jobs_lock_duration_ms).
 *
 * Pinned contracts:
 * 1. v132 exists in MIGRATIONS with the canonical name, idempotent flag, and
 ...
 * 3. SQL-level idempotency: re-executing the v132 statements directly on an
 ...
 * 5. A pre-v132-shaped table (column dropped) gains the column on re-run.
 */
...
const V130_SQL = MIGRATIONS.find(m => m.version === 132)?.sql ?? '';
...
describe('migration v132 — structure', () => {
  test('exists with canonical name, idempotent flag, engine-agnostic sql', () => {
    const v130 = MIGRATIONS.find(m => m.version === 132);
    ...
    expect(LATEST_VERSION).toBeGreaterThanOrEqual(132);
  });
...
describe('migration v132 — semantics (PGLite)', () => {
  ...
  test('pre-v132 shape (column dropped) gains column + CHECK on re-run', async () => {

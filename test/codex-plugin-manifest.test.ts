...
describe('marketplace shape (deliberately asymmetric — prove-before-publish)', () => {
  // dent-brain fork: `.claude-plugin/marketplace.json` is owned by this
  // fork's OWN `bun run build:plugin` (src/dent branding, the Cowork
  // marketplace teammates actually install — see check:plugin-version and
  // docs/dent-brain/*). It never carries upstream's generic "gbrain" +
  // gbrain-coding/gbrain-daily persona-variant shape these three tests pin.
  // `.codex-plugin/marketplace.json` (test above) still matches upstream
  // unchanged, since the fork doesn't touch that path.
  test.skip('claude marketplace: full plugin + the two persona variants', () => {
  ...
  test.skip('marketplace variant entries match the personas block exactly', () => {

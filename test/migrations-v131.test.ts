/**
 * Migration v133 (drop_job_wide_subagent_tool_use_id_unique) — #4155.
 *
 ...
 *    132 → runMigrations applies v133, the constraint is gone, and the same
 *    duplicate insert succeeds. Re-run applies nothing, and re-executing the
 *    v133 SQL directly is a no-op (DROP CONSTRAINT IF EXISTS).
 */
...
const V131_SQL = MIGRATIONS.find(m => m.version === 133)?.sql ?? '';
...
describe('migration v133 — structure', () => {
  test('exists with canonical name, idempotent flag, engine-agnostic sql', () => {
    const v131 = MIGRATIONS.find(m => m.version === 133);
...
describe('migration v133 — upgrade from a pre-v133 brain (PGLite)', () => {
  test('drops the constraint; duplicate insert throws before (positive control) and succeeds after', async () => {
    // Simulate a pre-v133 brain: re-add the job-wide constraint.
    await engine.executeRaw(
    ...
    // Apply v133 via the real migration runner (ledger rewind).

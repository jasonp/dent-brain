/**
 * Jobs fix wave (upstream issues #2/#3) — migration v130
 ...
 * 4. Ledger round-trip: rewind to 129 → runMigrations applies v130; re-run
 *    is 0 applied. AND (Codex C9) SQL-level idempotency: re-executing the
 *    v130 SQL directly changes zero rows — the ledger check alone only
...
const V128_SQL = MIGRATIONS.find(m => m.version === 130)?.sql ?? '';
...
describe('migration v130 — structure', () => {
  test('exists with canonical name, idempotent flag, engine-agnostic sql', () => {
    ...
    expect(v130?.name).toBe('minion_jobs_timeout_backfill_and_duplicate_cycle_cleanup');
...
describe('migration v130 — backfill + cleanup semantics (PGLite)', () => {
  test('full matrix: backfill hits legacy non-terminal mapped rows only; cleanup keeps newest ticker row per scope', async () => {
    ...
    // --- Apply v130 via the real migration runner (ledger rewind). ---
    await engine.setConfig('version', '129');
    ...
    // SQL-level idempotency (Codex C9): re-execute the v130 SQL directly and
  ...
  test('empty table: v130 SQL is a 0-row no-op', async () => {
    const before = await snapshot();

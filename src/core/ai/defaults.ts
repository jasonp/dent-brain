 ...
 *
 * FORK DIVERGENCE (dent-brain): upstream v0.48.2 answered the ZeroEntropy
 * sunset with Voyage `rerank-2.5`; this fork had already shipped Cohere
 * `rerank-v3.5` (see the v0.49.x CHANGELOG entry) and stays on it. Cohere's
 * v2 rerank endpoint is wire-compatible with the gateway's existing rerank
 * path, so this is a recipe choice and nothing more — upstream's surrounding
 * machinery (fail-open `no_key` contract, `reranker_skipped` search-meta
 * stamp, doctor readiness surfaces) is adopted unchanged. Deliberately kept
 * IDENTICAL to the runtime default below so the #3657 seam holds and init
 * never writes an explicit `search.reranker.model` row equal to the bundle
 * value (which would only earn a doctor `search_mode` reset nag).
 *
 * Score-scale caveat carried from the fork's own cutover: Cohere normalizes
 * `relevance_score` to a non-linear [0, 1]; rank-based logic is unaffected,
 * absolute cutoffs tuned against zerank-2 are not. Switching this back to
 * `voyage:rerank-2.5` is a one-line edit — every surface resolves through it.
 */
export const NEW_INSTALL_DEFAULT_RERANKER_MODEL = 'cohere:rerank-v3.5';
 ...
 * `search.reranker.model` is configured. v0.48.2 flipped it off the
 * sunsetting ZeroEntropy zerank-2 ahead of the 2026-09-04 hosted shutdown;
 * this fork's replacement is Cohere rerank-v3.5 (see the constant above). Keyless brains fail open per search

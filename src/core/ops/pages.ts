...
const get_page: Operation = {
  name: 'get_page',
  description: 'Read a page by slug (supports optional fuzzy matching). CAN BE LARGE — entity pages may exceed 50K tokens. For answering a question about a page (who/what/when), prefer `query` (hybrid search returns just the relevant chunks) or `get_timeline` / `get_backlinks` (structured slices). Use `get_page` only when you genuinely need the full body — e.g. you are about to edit it. Slug aliases left by renames redirect to the canonical page in the source that owns the alias (archived sources excluded); a redirected read reports `resolved_slug`. To edit a page, pass include_content: true — the returned `content` field is the canonical full markdown (frontmatter + body + timeline sentinel); edit THAT and pass it back to put_page to round-trip losslessly. Reassembling compiled_truth/timeline by hand risks dropping sections. Soft-deleted pages are hidden by default; pass include_deleted: true to surface them with deleted_at populated (see v0.26.5 recovery window).',
  params: {
...
const get_page_by_identity: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // Same federated-first precedence as get_page (#3242): federatedSearchScope
    // honors sourceIds[] for federated: true sources, not just scalar sourceId.

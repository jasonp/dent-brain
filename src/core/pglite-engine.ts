...
export class PGLiteEngine implements BrainEngine {
  ...
  async hasStaleSignaturePages(opts: { signature: string; sourceId?: string; includeNullSignature?: boolean }): Promise<boolean> {
    // Predicate is deliberately identical to invalidateStaleSignatureEmbeddings'
    // page-side clause — INCLUDING the includeNullSignature widening. If these
    // two ever drift, the gate starts hiding real work from the sweep: it would
    // answer "nothing stale" while the sweep would have invalidated rows, and
    // those chunks stay in the old embedding space with nothing surfacing it.
    // That is not hypothetical — the upstream sync that brought
    // includeNullSignature here (#3391) would have introduced exactly that gap
    // had the flag been threaded into the sweep alone. Pinned by
    // test/embed-stale-signature-gate.test.ts.
    ...
    const sigClause = opts.includeNullSignature
      ? `(embedding_signature IS NULL OR embedding_signature <> $1)`
      : `embedding_signature IS NOT NULL
          AND embedding_signature <> $1`;
    const { rows } = await this.db.query(
      `SELECT 1
         FROM pages

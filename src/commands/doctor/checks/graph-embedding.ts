  ...
  try {
    ...
    if (onSunsetReranker) {
      parts.push(
        ...
        `Fix: gbrain config set search.reranker.model ${NEW_INSTALL_DEFAULT_RERANKER_MODEL} (needs COHERE_API_KEY), or disable: gbrain config set search.reranker.enabled false.`,
      );
    }
    if (onSunsetColumns) {
      parts.push(

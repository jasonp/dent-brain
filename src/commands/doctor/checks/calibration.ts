...
export async function checkRerankerHealth(engine: BrainEngine, now: Date = new Date()): Promise<Check> {
  try {
    ...
    // with no row at all, and the default model is keyed on COHERE_API_KEY.
    ...
    if (authFails.length > 0) {
      return {
        name: 'reranker_health',
        status: 'warn',
        message: `${authFails.length} reranker auth failure(s) in last 7 days (key present but rejected). Fix: verify the reranker provider's API key (e.g. ${readiness.requiredKey ?? 'COHERE_API_KEY'}) and run \`gbrain models doctor\`.`,
      };
    }
    ...
    if (unknownFails.length >= 3) {
      const setupHint = unknownFails.some((f) => {
        ...
        return (
          ...
          summary.includes('COHERE_API_KEY') ||
        summary.includes('VOYAGE_API_KEY') ||
          summary.toLowerCase().includes('api key')
        );
      })
        ? " Fix: verify the reranker provider's API key (e.g. COHERE_API_KEY) and run `gbrain models doctor`."
        : '';
      return {
        name: 'reranker_health',
        status: 'warn',

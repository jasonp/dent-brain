...
async function embedAllStale(
  ...
  //
  // #4283: once the cheap probe says there IS drift, NULLing is still
  // conditional on a WORKING embedder (bad key, unreachable provider,
  // wrong-dims model) — a failed probe skips the invalidation so a
  // misresolved config can't strip vectors it can never replace.
  // validateEmbeddingCreds at runEmbedCore entry is a static env check only —
  // the probe is the live proof.
  if (!dryRun && signature) {
    const scopeOpt = {
      signature,
      ...(sourceId && { sourceId }),
    ...
    // v0.49.4.0 gate: the sweep below joins pages against every content_chunks
    // row, so it costs a full scan of the largest table whether or not anything
    // is stale. The probe asks the same question against pages alone. It takes
    // the SAME opts object, so includeNullSignature widens both or neither —
    // threading the flag into only the sweep would make the gate answer "no
    // work" while the sweep had work, stranding chunks silently.
    let invalidated = 0;
    if (await engine.hasStaleSignaturePages(scopeOpt)) {
      ...
      if (probeOk) {
        invalidated = await invalidateStaleSignatureEmbeddingsGuarded(engine, scopeOpt);
      } else {
        serr(
          `  [embed] WARNING: chunk(s) drifted from signature ${signature} but the ` +

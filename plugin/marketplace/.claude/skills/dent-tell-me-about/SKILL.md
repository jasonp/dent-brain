...
Entity pages in Distributed Brain can be huge — `entities/people/sumanth-channabasappa.md` is 80KB+, attendees with multi-year history easily exceed 100KB. Calling `get_page` on these dumps the entire compiled body into the context window. That's wasteful and tells the user nothing the brain hasn't already indexed.
...
The user typed something like "Alice Chen" or "Acme Conf 2026" or "Granola". Resolve it:
...
- `entities/projects/<slug>` — initiatives + specific events (Acme Conf 2026, Ketchum Library Talk, etc.)
...
- **Match in `entities/audience/` only** — note this in the response: "I have an email-list contact for X but no org event history yet."
...
### Shape A: open-ended summary ("tell me about Alice", "who is Sarah?")
...
### Shape B: specific factual question ("what's Alice's role?", "where does Sarah work?")
...
### Shape C: relationship question ("who has Alice met with?", "who works at Acme?")
...
### Shape D: timeline / history question ("when did Alice register for Acme Conf 2024?", "how many events has X attended?")
...
- **Don't dump the whole timeline if the user asked a specific question.** A "what's Alice's email" question doesn't need 30 registration bullets — it needs the frontmatter + maybe one bullet.

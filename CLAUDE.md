...
- **Long-flag reads go through `readFlagValue` / `readFlagValues`**
  (`src/core/cli-flag-value.ts`). Never hand-roll argv scanning in a CLI_ONLY command: most
  hand-rolled spellings match only the space-separated form and silently DROP the
  equals-joined one, so the command falls through to ambient defaults — for `sync --source=`
  ...
  `cli-flag-registry.generated.ts`. Guarded by `test/cli-flag-idiom-guard.test.ts` +
  `test/cli-flag-validation.test.ts` — `bun run verify` runs NEITHER, so run them explicitly
  after touching flag parsing. **The idiom guard is not yet complete**: it matches two
  spellings, the sweep has converted six files, and seven commands still read `--source` in a
  spelling it misses (P1 in TODOS.md). Treat a green guard as necessary, not sufficient —
  when you touch a command's flag parsing, convert its reads while you are in there.

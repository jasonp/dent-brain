...
- [ ] **P2 — Decide how to stop one teammate's Gmail burst from rate-limiting everyone.** All teammates share one "Dent Brain" Google Cloud OAuth app — **published, restricted to the internal team** (corrected 2026-08-11; earlier notes wrongly said *test mode*). Gmail per-project quota is therefore pooled, so one teammate's burst throttles everyone (this is the structural cause behind the v0.46 429 lockout — the code fixes make a 429 survivable and now self-clearing, but don't raise the ceiling). Two options, pick one (needs Jason's Google account): **(a)** request a Gmail API quota increase in the GCP project (GCP Console → APIs & Services → Gmail API → Quotas — check whether the limit hit is per-user or per-project first); **(b)** issue each teammate their own OAuth client so bursts are isolated (most robust, most onboarding change — touches `dent-onboard-teammate` + install env). The old "move from test to production/verified" option is moot — the app is already published. Filed from the v0.46 bug report §B7.
...
- [ ] **Adopt upstream's parallel `verify` (`scripts/run-verify-parallel.sh`, currently exposed as `verify:parallel`).** Blocked by two real gaps surfaced during the v0.43 sync: (a) the `check:proposal-pii` npm-script alias is missing (the dispatcher's CHECKS array references it but it's undefined in `package.json`); (b) `check:operations-filter-bypass` fails because `src/dent/serve.ts` imports operations but isn't in that gate's ALLOWED list — it needs either a `.filter(op => !op.localOnly)` at the import boundary or an allowlist entry with a one-line rationale (verify the trust boundary first — serve.ts is the remote-facing MCP transport). The fork kept its inline `verify` (which carries `check:plugin-version` / `check:newlines` / `check:exports-count`) so /ship's gates stay intact. Once the two gaps are closed, switch `verify` to the parallel dispatcher and update `test/privacy-script-wired.test.ts` back to the delegation assertion. **Update v0.45.0.0: gap (b) is closed — serve.ts now applies `.filter(op => !op.localOnly)` at the import boundary and is in the gate's ALLOWED list with its own enforcement check. Only gap (a) (`check:proposal-pii` alias) remains.**

- [ ] **Three inherited-upstream E2E reds (not fork bugs — fix-forward or wait for upstream + re-sync).** (a) `test/e2e/dream-cycle-phase-order-pglite.test.ts` — upstream's `ALL_PHASES` in `src/core/cycle.ts` includes `enrich_thin` (v0.42.6.0) but their `EXPECTED_PHASES` test constant omits it; one-line fix is to add `enrich_thin` to `EXPECTED_PHASES`. (b) `test/e2e/sync-lock-recovery.test.ts` — upstream's code implements `sync --break-lock --all` (iterate sources, exit 0) while their own test expects refusal (exit 1); needs a behavior-vs-test judgment (does upstream intend `--all` support or refusal?). These reproduce on pure upstream/master; the merge is faithful. Filed during the v0.43.0.0 ship.

## RLIMIT_NPROC fork exhaustion wedges the server (root-caused + fixed in v0.45)

- [x] **v0.40.x: Find and fix the subprocess leak that exhausts the container's process table.** On 2026-06-02 the production Railway container reached a state where it `can't fork` — every MCP markdown write returned a generic "Internal error", the scheduled `performSync` loop silently stalled, and even `railway ssh` commands failed with `sh: can't fork: Resource temporarily unavailable`. Cleared only by `railway redeploy` (fresh process table); root cause never found. The Dockerfile already has partial mitigation (tini as PID 1 + a Node-side SIGCHLD reaper installed in `src/dent/serve.ts`, added in v0.34.2) yet exhaustion still happened, so something is leaking processes/zombies the reaper can't reach — likely a native-addon spawn or a git/ssh/rev-list child path that isn't waited on. Investigate: (a) which code spawns subprocesses on the hot path (sync git pull/rev-parse via `src/core/git-remote.ts` + the legacy `git()` helper, embedding/extract child processes, tree-sitter wasm, image decoders); (b) add a periodic process-count log + a doctor check that warns when the container's child/zombie count climbs; (c) reproduce by hammering the write+sync path in a loop against a container with a low `RLIMIT_NPROC`. Note this compounded the 2026-06 SLUG_MISMATCH freeze — the sync-resilience fix (v0.40.0.1, PR #26) handles the parse-failure freeze, but a wedged fork table is an independent failure mode that still requires a manual redeploy. Filed 2026-06-02 during the v0.40.0.1 ship; needs a repro before it's actionable. **Update 2026-06-12: recurred in production and root-caused.** Railway logs showed every git child failing (`fatal: fork failed: Resource temporarily unavailable`, `unable to create threaded lstat`) across the regfox tick, markdown-writer cron, and pulls — the per-write git fork pattern itself exhausts the container over time. This is what made `markdown_append_to_page` fail 5/5 on a board member's page (docs/issues/markdown-write-internal-errors-and-large-page-splice.md), not page size. Cleared by redeploy. Permanent fix is the v0.45 single-brain work: retire the git dual-write so the server stops forking git per write entirely.
  **Completed:** v0.45.0.0 (2026-06-12) — root cause was the per-write git child-process pattern itself (every markdown write + regfox tick + 5-min scheduled pull forked git; under load the container exhausted its process table). v0.45 removes git from the write path entirely; the only remaining git children are the once-nightly exporter + boot clone.

## entity-detection: first-name-fallback false-matches common words

- [ ] **Stop `detectEntities` first-name-fallback from firing on common English words and place names.** `src/dent/entity-detection/service.ts` pass 3 (first-name-fallback, confidence 0.7) matches any person page's first token as a whole word. Observed false positives on 2026-05-19: "Austin" (the city) → `entities/people/austin-nafziger`, "will" (the modal verb) → `entities/people/will-tracy`. The `STOPWORDS` set (service.ts:37-45) only covers days/months/yes-no/Dent — it doesn't cover common English words that happen to be names. Two candidate fixes: (a) expand STOPWORDS with a common-English-word + common-place-name list (cheap, blunt — risks suppressing a real "Will" mention), or (b) make first-name-fallback opt-in per entity via a frontmatter flag (e.g. only fire fallback for pages with `firstname_fallback: true` or an explicit `aliases:` entry), so the founders' aliases already cover them and randoms don't auto-match. Option (b) is cleaner and composes with the founder-alias work landed in dent-brain-data f2dd38c1 (2026-05-20). Add a regression test in the entity-detection test file asserting "Austin"/"will" produce no person match while "Jason"/"Steve" still resolve via alias. Filed during the founder-alias-priority work.

## Windows support for email-sync (v0.40 — own PR)

- [ ] **v0.40.x: Cross-platform email-sync for the Windows teammate.** Scope = email-sync only; granola-sync stays Mac-only (Granola.app doesn't run on Windows). **Reviewed via /plan-eng-review 2026-05-22 — approach locked:**
  - **Installer:** unified Bun installer — port `tools/email-sync/install.sh` → `install.ts` with a **linear OS-agnostic core** (MCP discovery → config write → OAuth → recipe copy) calling a small platform module (resolved by `process.platform`) only at 3 branch points: scheduler-file render (plist vs Task Scheduler XML), secrets ACL (chmod 0600 vs NTFS verify), path expansion (`$HOME` vs `%USERPROFILE%`). No per-OS duplication. Granola keeps `install.sh`.
  - **Scheduler abstraction** in `tools/extensions/scheduler/` (fork-only): `Scheduler` interface (arm/disarm/isLoaded/status) with `LaunchdScheduler` (lift existing `launchctl` logic out of `cli.ts:43,348,376` verbatim — behavior-identical, re-verify Mac after lift) + `TaskSchedulerScheduler` (`schtasks /create /xml`, `/delete`, `/query`). `cli.ts` picks by `process.platform`. **arm MUST check the `schtasks` exit code** (mirror the launchd bootstrap exit check at `cli.ts:350`) or a failed Windows registration looks armed but never runs.
  - **NO macOS-keychain port needed:** email-sync stores Google OAuth tokens in a plain file (`google-tokens.json`, chmod 0600), not the keychain. On Windows: verify NTFS ACL restricts to the user; warn on fail. (The keychain is granola-only, and granola stays Mac-only.)
  - **Registry:** add `platform?: ('darwin'|'win32')[]` to the `Extension` interface (`tools/extensions/types.ts`); tag `granola-sync: ['darwin']`, `email-sync: ['darwin','win32']`; add `'scheduled-task'` to `ExtensionKind`. CLI `install` refuses with a clear message when `process.platform` not in `platform[]`.
  - **Tests:** under `test/dent/ingestors/` (**fork-only** — upstream has 0 files there; keeps Windows tests out of the upstream-shared `test/` root). Mac-testable seam = pure generation: assert rendered `schtasks` argv + Task XML against fixtures, plus registry platform-gating. Windows runtime exec is teammate dogfood only — no Windows box to dogfood against locally.
  - **Lightweight-layer verdict:** all production work is in `tools/` (verified fork-only — upstream `master` has no `tools/` tree), so future `bun run sync:upstream` never touches it. Plan adds **zero** divergence to upstream-shared files (`src/cli.ts` and `package.json` untouched). Recipe model (`user/filter.ts`) stays unchanged.
  - Prereqs to confirm with teammate: Bun installed on their Windows box (or compile a `bun-windows-x64` `.exe` — deferred sub-decision); their work email allowlisted in the shared Google OAuth app.

- [ ] **P3 — Unify granola-sync onto `install.ts` (after email-sync Windows port ships).** Once email-sync's `install.ts` proves out, port granola-sync's `install.sh` to the same install.ts pattern (Mac-only branch) so there's one installer paradigm, not two. Prevents bash/TS drift. Low priority — granola-sync works today on its `install.sh`. Filed from the v0.40.x Windows /plan-eng-review.

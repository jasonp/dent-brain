...
- **Reranking pair**: Cohere `rerank-v3.5` is the hosted default (on for `balanced` and `tokenmax`). It replaced ZeroEntropy `zerank-2` in v0.49.0.0 — ZeroEntropy shuts down 2026-09-04, so do not start new work against it (see [`docs/ai-providers/zeroentropy.md`](../ai-providers/zeroentropy.md)). Voyage `rerank-2.5` pairs cleanly with Voyage embeddings.

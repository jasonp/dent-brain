...
# dent-brain-fork: bun run build:plugin layers a Cowork marketplace bundle
# (plugin/marketplace/, plugin/manifest.json, plugin/fm-mcp/) on top of
# upstream's generate-plugin-tree.ts output. Those paths are fork-owned and
# don't exist in $TMP (the generator only ever produces README.md + skills/),
# so they're excluded from the drift comparison rather than disabling the
# check upstream's own skills/README drift detection still covers.
PLUGIN_TREE_EXCLUDES=(-x marketplace -x manifest.json -x fm-mcp)
if ! diff -r "${PLUGIN_TREE_EXCLUDES[@]}" "$TMP/plugin" "$ROOT/plugin" >/dev/null 2>&1; then
  ...
  diff -rq "${PLUGIN_TREE_EXCLUDES[@]}" "$TMP/plugin" "$ROOT/plugin" 2>&1 | head -20 >&2
  exit 1
fi

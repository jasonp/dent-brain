...
export const EXTENSIONS: Extension[] = [
  {
    ...
    kind: 'launchd-daemon',
    // Granola.app is macOS-only (it mints the API key + stores it in the
    // macOS keychain), so granola-sync stays Mac-only. The CLI refuses to
  ...
  },
  {
    ...
    kind: 'launchd-daemon',
    // email-sync is pure-API (Gmail over OAuth) with file-based token storage
    // (no keychain), so it runs on both macOS (launchd) and Windows (Task
    // Scheduler). The scheduler abstraction picks the mechanism by platform.
    platform: ['darwin', 'win32'],
    sourceDir: 'tools/email-sync',
    // Cross-platform Bun installer. cmdInstall runs `.ts` via bun (any OS);
    // granola-sync's `.sh` still runs via bash (Mac-only).
    installScript: 'install.ts',
    ...
    taskName: 'DentBrain-EmailSync',
    taskXmlPath: '${HOME}/.dent-brain/email-sync/email-sync.task.xml',

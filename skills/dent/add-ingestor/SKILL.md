...
1. **Cowork conversational capture** — `/{{prefix}}-append-evidence`. Already wired (this is Phase 1+2 work).
...
- **Inbound auth.** RegFox webhook secret, Gmail OAuth refresh token, Dropbox OAuth, etc. Stored as Railway env vars (`{{prefix}}_REGFOX_WEBHOOK_SECRET=…`).

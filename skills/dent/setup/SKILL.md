...
8. **Install the Cowork plugin** — add the local marketplace + install the plugin so /{{prefix}}-* slash commands work in Cowork.
9. **Run a smoke-test** — append an observation about a test entity, observe a real commit in the data repo within seconds.
10. **Onboard teammates** (optional) — run `/{{prefix}}-onboard-teammate` for each.
...
If all 10 steps are done, congratulate them and ask if they want to onboard a teammate (which routes to /{{prefix}}-onboard-teammate).
...
1. Create a new GitHub repo at the path they entered for `data_repo` (e.g., `github.com/<their-org>/{{prefix}}-brain-data`). Repo can be private or public; the server's deploy key gives it write access regardless.
...
1. `ssh-keygen -t ed25519 -f /tmp/{{prefix}}-deploy-key -N '' -C '{{prefix}}-server@railway'`
2. Add the public key (`/tmp/{{prefix}}-deploy-key.pub`) to the data repo's GitHub Settings → Deploy keys with **Allow write access** checked.
3. Set the private key on Railway: `railway variables --set "DENT_BRAIN_DATA_DEPLOY_KEY=$(cat /tmp/{{prefix}}-deploy-key)"`.
4. `rm /tmp/{{prefix}}-deploy-key{,.pub}`.
...
- The admin opens a fresh Cowork chat and asks: *"Add a custom marketplace from `github:<their-code-repo>` and install the `{{prefix}}-brain` plugin."*
...
The admin runs `/{{prefix}}-append-evidence` from Cowork against a known entity. Expected result:

- Cowork responds with a `commit_sha`.
- `git log` in the data repo (after `git pull`) shows a commit by `{{prefix}}-brain-server <noreply@<email-domain>>` with message `agent: append entities/people/<slug>`.
- The new bullet appears in the markdown file.

### Phase 9. Onboard teammates

Optional. For each teammate:
- Run `/{{prefix}}-onboard-teammate` — generates a per-user bearer token, produces a one-paste install command.
...
Optional. Run `/{{prefix}}-add-ingestor` to add a signal source (RegFox, Gmail, Granola, Dropbox). These are server-side workers that translate external signals into `markdown_append_to_page` calls. Not required for the brain to function — Cowork captures alone are useful — but they amplify the input pipeline.
